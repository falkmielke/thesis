...
The transformation enables researchers to tackle a standing problem: joint angles have been associated with issues of transferability citep:Gatesy2011, owing to the fact that they are influenced by morphological- and other constraints.
...
For example, citet:Gatesy2011 assess that motion transfer between a human and flamingo fails, causing "undesirable motion artifacts" due to morphological discrepancy.
...
Transformation to the frequency domain enables the isolation of affine components of joint angle profiles, and the extraction of measures of coordination.
With FCAS, one could compare relative joint angle profiles (e.g. hip joint superimposition to inspect the knee joint, as in Ch. \ref{cpt:fcas}), or one could specifically adjust dynamic posture and attempt to only transfer coordination (Ch. \ref{cpt:piglets}).
One could take a broader phylogenetic perspective and assess which morphological innovations induce changes in posture, range of motion, or coordination.
...
Applying a predictive model of NBW kinematics to LBW is an implicit form of motion transfer: which age/size would we assign to an NBW which walked like the LBW we observe?
FCAS enables predictive modeling at the construct level of locomotion.
It allows for the separation of different aspects of kinematics (e.g. coordination), potentially making some of these transferable.
...
The authors digitized an enormous number of strides of different species of elephants@@latex:\footnote{For reference: this was at a time when high-end consumer GPUs housed about 256MB of memory; long before the era of DeepLabCut.}@@.
...
By now, these modeling tasks are not possible, because joint angle profiles and their variability are difficult to capture numerically, and in consequence reconstruction remains vague or non-quantitative.
...
Not a single stride could be considered "representative", because young piglets in our setup literally always tumbled, jumped, sidestepped, stopped shortly after, or interfered with each other.
