...
+ FCAS enables *motion transfer within* a sequence of joints: some or all affine components can be transfered to adjacent joints (Ch. \ref{cpt:fcas}), leaving the residual, relative movement, i.e. coordination.
...
The "age" model provided evidence that maturation of LBW locomotion halts aproximately four hours /post partum/ (developmental delay).
...
Therefore, though it seems tempting to turn to piglets in light of all the prior research, it might be questionable to choose them as a model /for musculoskeletel aspects/, or even for behavioral aspects which rely on that system.
