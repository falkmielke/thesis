...
Note that these observations are restricted to walking gait at voluntary speed; we cannot exclude more obvious LBW/NBW differences at more challenging motor tasks.
However, the apparent developmental delay in even this basic motor skill indicates the high sensitivity of the method.
Also, the relevance of stair- or treadmill studies must be questioned for animals whose major priority is to find and access their mother's teats.



#+CAPTION: XROMM using python and blender. Full video ([[https://ody.sh/fZe3OpgHGN]]) and code ([[https://git.sr.ht/~falk/foss_xromm]]) are available.
...
This was only evident after performing actual 3D measurements.
Not only are the recordings variable, but of the 392 recordings we obtained in several recording sessions in summer 2019, not a single one showed locomotion which strictly fulfilled the "steady state" criterium and gave single limb forces.
...
The XROMM procedure is a low through-put method, and it was not possible to digitize a sufficient number of strides to assess LBW differences in joint moment orchestration.
Finally, the software conventionally used for XROMM caused issues: due to its large dependency tree and lack of documentation, XMALab (Brown University, USA, https://xromm.org) was dysfunctional for about half a year during this PhD; Maya AudoDesk is non-free software.
...
Except for ORS Dragonfly (Object Research Systems, Canada, https://www.theobjects.com), which was used for CT segmentation but could be replaced by 3D Slicer (Slicer Community, https://www.slicer.org), all the tools in the workflow are free and open source.
The example animation can be viewed in blender from any perspective, and replayed at self-chosen speed.
However, segment position glitches occur when markers leave the focal volume of the biplanar x-ray, joint forces and moments do not seem to withstand a visual quality check (their direction and magnitude are questionable), and thus an error in the computational pipeline cannot be excluded.
...
Though differences in /why/ segments move in a particular way are unlikely in the light of this finding, methodological limitations prohibited definitive conclusions on dynamic differences in the study groups.


* Piglets
limited conclusions for human development
