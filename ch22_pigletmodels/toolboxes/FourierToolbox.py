...
class Limb(dict):
    ...
    def Plot(self, ax, title = None, subset_joints = None, *args, **kwargs):
        ...
        store = {} # return the reconstruction
        for joint, trace in self.items():
            ...
            if 'label' in kwargs.keys():
                ...
                label = f"{self.label}, {joint}"

            # plot!
            y = trace.Reconstruct(x_reco = self.time)
            # if (NP.mean(y) > 0) and (joint == 'metacarpal'):
            #     y -= 2*NP.pi
            # if (NP.mean(y) < 0) and (joint == 'metatarsal'):
            #     y += 2*NP.pi

            # if (joint == 'metacarpal') and (NP.mean(y) > 4):
            #     print (self.label, joint)

            ax.plot(self.time, y, label = label, *args, **kwargs)

            store[joint] = y

        # MPP.legend() # should be done by master
