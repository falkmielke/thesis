...
In the size model, there are some positive and some negative effects which seem cancel out (which might be evidence for size-dependent anternative locomotor strategies; Figs. \ref{fig:input_distributions}, \ref{fig:output_effects}).
...
\begin{figure}[p]
  \caption{ Observed model parameter input distributions (\textit{next page}).
...
\caption{\label{fig:input_distributions}Observed model parameter input distributions. \textit{See previous page.}}
\end{figure}
...
\caption{\label{fig:output_effects}Effects of each input parameter on the model output. \textit{See next page.} }
\end{figure}
\addtocounter{figure}{-1}
\begin{figure}[p]
  \caption{ Effects of each input parameter on the model output (\textit{previous page}).
...
Firstly, we can compare the input distributions (criterium 1, above) of the observational groups (Fig. \ref{fig:input_distributions}), to identify those model parameters which acutally influence the predictions.
Secondly, we can calculate the absolute effect of each parameter onto the predicted outcome (slope magnitude, criterium 2), by multiplying the parameter magnitude with the according slope (Fig. \ref{fig:output_effects}; this works because \(\Delta y \sim b_i \cdot \Delta x_i\)).
Thirdly, we can quantify the model residual (criterium 4; Fig. \ref{output_effects}).
