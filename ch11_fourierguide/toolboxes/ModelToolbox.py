...
def AddComponent(  model: PM.Model \
    ...
    with model:
        shared = PM.Data(f'{label}_data', data_matrix)
    ...
    component = TT.dot(shared, population)
...
def AddMultiVariateComponent( \
    ...
    with model:
        ...
        block = TT.stack(covariates, axis = 1)
        # PrintShape('covariates', block)

        component = TT.dot(shared, block)
...
def AddMultiVariateMultiLevelComponent( \
    ...
    with model:
        ...
        for obsl in range(n_observables):
            ...
            allgroups = TT.stack(groupwise, axis=0)

            # add up slopes for each of the groups
            obsn_params = TT.dot(shared_grp, allgroups) # take the right group for each row

            block = shared_dat * obsn_params # multiply data and slopes

            obslwise.append(TT.sum(block, axis = 1))
        ...
        #                                  , TT.stack(obslwise, axis = 1) \
        #                                  )


        obslwise = TT.stack(obslwise, axis = 1)
...
def AddPosterior( \
    ...
    with model:
        ...
        residual = PM.HalfCauchy('residual', prior_kwargs.get('residual', 1.), shape = n_observables)
    ...
    with model:
        ...
        if fixed_covariance:
            ...
            if False:
                ...
                cholesky_matrix, _, _ = PM.LKJCholeskyCov(  'packed_cholesky' \
                                                      , n = n_observables \
                                                      , eta = 1. #prior_kwargs.get('eta', 1.) \
                                                      , sd_dist = PM.HalfCauchy.dist(1.) \
                                                      , compute_corr = True \
...
#______________________________________________________________________
    def Preparation(self):
        ...
        for obs in self['observables']:
            ...
            self.mean_storage['sd'].append(sd0)
...
# model results and diagnostics
    def Hygiene(self, title = None):
        ...
        if self['robust']:
            var_names += ['dof']
        var_names = [f'{self.label}_{vn}' for vn in var_names]
    ...
    def Summary(self):
        ...
        for obsl_nr, observable in enumerate(self['observables']):
            for parameter in [v for v in self.settings.GetPredictors().index.values] + ['intercept']:
                ...
                observable_translation[f'{self.label}_{parameter}[0,{obsl_nr:.0f}]'] \
                                = [observable, parameter]

            # stride params
            for pred_nr, pred in enumerate(preds):
                observable_translation[f'{self.label}_stride_population_{observable}[{pred_nr:.0f}]'] \
                            = [observable, pred]
            ...
            for ac in range(3):
                for pred in ['lbodymass', 'lleg_m', 'lbmi']:
                    observable_translation[f'{self.label}_{pred}_ml_ageclass_{observable}[{ac},0]'] \
                        = [observable, pred]
                    observable_translation[f'{self.label}_c{pred}_ml_ageclass_{observable}[{ac},0]'] \
                        = [observable, f'c{pred}']

            # for the ml intercept case
            observable_translation[f'{self.label}_intercept_ml_population_{observable}[0]'] \
                            = [observable, 'intercept']

            # dof
            observable_translation[f'{self.label}_dof[{obsl_nr:.0f},0]'] = [observable, 'dof']
            observable_translation[f'{self.label}_dof'] = ['misc', 'dof']

            # cross correlation
            for obsl_nr2, observable2 in enumerate(self['observables']):
                observable_translation[f'{self.label}_packed_cholesky_corr[{obsl_nr},{obsl_nr2}]'] = ['misc', f'{observable}|{observable2}']

            # stds
            observable_translation[f'{self.label}_packed_cholesky_stds[{obsl_nr}]'] = [observable, 'std']
            ...
            with self.model:
                ...
                if PM.__version__ == "4.0":
                    ...
                    while count < n_samples:
                        ...
                        sampling.append( \
                                    PM.sample_posterior_predictive(  self.trace \
                                                                  , samples = 1 \
                                                                  , progressbar = False \
                                                                  )[f'{self.label}_posterior'] \
                                         )
...
observable_translator = { \
                         ...
                         , 'lbmi': 'log bmi (kg/m)' \
                          ...
                          , 'μ iknee': 'knee angle' \
                          , 'α iknee': 'knee amplitude' \
                          , 'μ iankle': 'ankle angle' \
                          , 'α iankle': 'ankle amplitude' \
                          }
...
def GetPosteriorCorrelation(model):
    ...
    row_selection = [idx for idx in summary.index.values \
                     if 'packed_cholesky_corr' in idx[1]]
...
def GetPredictorEffects(model):
    ...
    if model['obscov_fixed']:
        corr, cov, chol, std, dmat = CalculateCorrelation(model.data, model['observables'], model['intercept_level'])
        print (std, model['observables'])

    results = PD.DataFrame(index = model['observables'] \
                           , columns = predictor_columns)

    for obsl_nr, observable in enumerate(model['observables']):
        # print ('\n####', observable)
