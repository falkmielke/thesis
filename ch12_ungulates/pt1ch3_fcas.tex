...
Gatesy and Pollard \citep[2011, ][]{Gatesy2011} have attributed the limited quantitative accessibility of coordination to limitations in transferability of kinematic profiles.
...
One of them could be phylogenetic signal, which we found to be less relevant (supplement \ref*{apdx:phylosig}).
...
\textit{Movie:} {An example recording from a walking lama, obtained by the authors with permission by the owner. Tracked landmarks (shoulder, elbow, carpus, fetlock) are superimposed; joint angle profiles for four consecutive steps are shown below. The video is available in high quality here: \url{https://www.youtube.com/watch?v=J1fhg33ZDvI}. }\label{supp:movie1}


\textit{dataset:} {supplementary\_tutorial.zip}{A step-by-step tutorial (formats: HTML and jupyter notebook) that illustrates the implementation and application of the methods presented in this manuscript.}\label{supp:tutorial}


\textit{dataset:} {supplementary\_data1.xls}{A list of and link to the videos from which data was acquired, including timestamps of the stride cycle episodes that entered the analysis. }\label{supp:data1}


\textit{dataset:} {supplementary\_data2.zip}{All tracking data generated from the videos. Note that many stride cycles did not pass quality criteria for the final data set. }\label{supp:data2}
