...
\cfoot{\text{ \thepage }}
\lfoot{\text{ }}
\input{org/latex_definitions.tex}
\author{Falk Mielke \href{mailto:falkmielke.biology@mailbox.org}{(falkmielke.biology@mailbox.org)}}
\date{2021-11-02}
\title{Memorandum: Probabilistic Modeling of Baboon Kinematics\\\medskip
\large -- a tiny textbook about a fraction of statistical methodology --}
\hypersetup{
 pdfauthor={Falk Mielke \href{mailto:falkmielke.biology@mailbox.org}{(falkmielke.biology@mailbox.org)}},
 pdftitle={Memorandum: Probabilistic Modeling of Baboon Kinematics},
 pdfkeywords={},
 pdfsubject={},
...
The data set has been analyzed before \citep[currently under review, ][]{Druelle2021prep}, and the purpose of another upcoming manuscript is to extend the analysis to questions of \emph{posture} and \emph{coordination}.
...
\begin{tabular}{|l|l|l|}
\hline
 & \textbf{Frequentist} & \textbf{Probabilistic}\\
\hline
\textbf{Hypotheses} & decision trees & ``Bayesian'' hyp. tests\\
\hline
\textbf{Modeling} & (least squares) regression & MCMC Sampling\\
\hline
\end{tabular}
\end{table}
...
The approach presented in this document is \textbf{probabilistic modeling}.
...
\begin{tabular}{llll}
\hline
\textbf{category} & \textbf{parameter} & \textbf{units} & \textbf{description}\\
\hline
\hline
...
In total, \texttt{40} stride cycles from \texttt{17} subject individuals entered the analysis.
...
The first two principal components capture \texttt{99.8 \textbackslash{}\%} of the variance, justifying an optional reduction to two parameters via PCA transformation.
...
Of the \texttt{17} subjects included, \texttt{5} were male.
...
A total of
\texttt{4} adults (\texttt{4/0} females and males, respectively),
\texttt{6} (\texttt{4/2}) adolescent
and \texttt{7} (\texttt{4/3}) infant
...
The \emph{bodymass} of the animals was determined for each recording (averages for infant/adolescent/adult individuals: \texttt{3.0/10.2/13.1\textbackslash{} kg}).
...
The two leg long bone segments (hip-knee-ankle) were chosen as a size reference for normalization of distance values. They measured \texttt{0.27/0.41/0.43\textbackslash{} m} (inft./adol./adult) per ageclass on average.
Adding the length of the torso segment (head-hip), we retrieved a total body length proxy, which was used to calculate a ``body mass index'' (BMI, averages \texttt{5.0/11.4/14.1\textbackslash{} kg/m} for the respective ageclasses).
...
\item \textbf{Data:}
\label{sec:org71fa6a6}
Because there is usually only one bodymass per subject animal in our data set, the data set onto which I apply the bodymass model is reduced (\texttt{17} observations).
For one animal which was measured at two bodymasses, the average value was used.

\item \textbf{Transformations:}
\label{sec:orgf39d02c}
...
\subsubsection{Bodymass is Age-Dependant (Surprise!)}
...
\hline
\textbf{parameter} & \textbf{value} & \textbf{interval} & \textbf{sig.}\\
\hline
...
On average, adolescents are \texttt{2.4\textbackslash{} kg} and infants are \texttt{9.4\textbackslash{} kg} lighter than adult individuals.
...
In contrast to age, sex seems to have no effect on bodymass (keeping in mind that there are generally few and no adult males in the sample; slope: \texttt{+0.6\textbackslash{} [-1.9, +3.7]\textbackslash{} kg} ).
...
For our bodymass model, the sampler converged at \texttt{\textbackslash{}nu = 52.1}.
...
It is high (\texttt{\textbackslash{}nu = 54.2}), indicating that the posterior distribution has little more ``weight in the tails'' than a Normal distribution would.
...
\centering
\begin{tabular}{rrrrl}
\textbf{rank} & \textbf{loo} & \textbf{delta} & \textbf{std err} & \textbf{description}\\
\hline
...
4 & 285.6 & 0.9 & 10.1 & \textbf{reference model}\\
...
The first \(8\) components capture \texttt{91.0 \textbackslash{}\%} of the variance and were submitted to the modeling procedure.
...
3 & 229.9 & 5.1 & 14.8 & \textbf{reference model}\\
...
4 & 330.6 & 6.7 & 14.8 & \textbf{reference model}\\
