...
We can witness the birth of another creature.
We can feel emotionally stirred by that encounter, empathize with the mother or the offspring, feel delight and astonishment.
We can be amazed by how little (or how much) time it takes for a little one to stand and walk, to /live/ independently.

We of course know about evolution, and that the variable processes around birth are strongly correlated to a species' survival.
This strong correlation is called "selection", a concept to which we owe some of the most puzzling and fascinating phenomena in nature.
...
Complexity triggers fascination, triggers emotions - both when things go right, and when things go a bit differently.
...
The limerick above might be considered poetic, you might find it beautiful, but on close read you will find that it is definitely not "art", and [[https://callingbullshit.org][ultimately it is AI-generated bullshit]] (Fig. \ref{fig:limerick}).
Does that change your opinion about it?

#+CAPTION: The "bing chat" prompt which lead to the limerick above (Nov 3rd, 2023). Besides the use of "DeepLabCut" for landmark tracking, this poem and the steampunk figura below are the only AI contributions to my thesis.
...
Locomotion is a highly complex phenomenon by itself, and the quick perinatal maturation of precocial animals turns out to be a methodologically challenging topic.
The backbone of this thesis is computational methodology, applied in a novel context.
...
Nevertheless, herein, I will not ponder further upon the emotional or artistic aspects of the beauty of nature.
The core purpose of this thesis is to assemble, extend, and scientifically evaluate the different facets of current research methods around locomotor development.


#+CAPTION: "A newborn steampunk piglet galloping on a farm with the sow in the background. (futuristic-biomechanical)", created by [[https://stablediffusionweb.com][the "stable diffusion" AI image generator (https://stablediffusionweb.com)]] on November 3rd, 2023. Impressive as is, this generated image has no deeper meaning or connection to real life, and its only fascinating aspect might be how it was generated. But enough of AI at this point.
#+ATTR_LATEX: :placement [p] :width 16cm
#+LABEL: fig:steampunk
...
To fundamentally solve this issue, it is important to take one step back, explore what locomotion is /technically/, and see which analytical methods are available.
\end{change}
...
And it is bound to classical, frequentist statistics, with all its assumptions and pitfalls.
...
Because it is an information-preserving method, morphology becomes a well-controlled model parameter.
