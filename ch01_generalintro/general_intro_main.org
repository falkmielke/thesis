...
For example, imagine you measured two maximally dissimilar animals, e.g. a stork and an elephant, while co-incidentally running at dynamically similar speed.
We measure exactly the same (normalized) speed.
...
\begin{change}
This observation is due to coordination: coordination is henceforth understood as the non-random temopral pattern that arises in the succession of movements of different joints within a limb (intra-limb coordination) or between limbs (inter-limb coordination).
\end{change}
Coordination is achieved by (neuro-)motor control.
\begin{change}
Control in the scope of this thesis is the central or peripheral nervous structure which establishes coordination.
\end{change}
...
+ ... before turning the conventional modeling strategy upside down and applying a predictive model to piglets @@latex:\citep[Ch. \ref{cpt:piglets},][]{Mielke2023}@@.
