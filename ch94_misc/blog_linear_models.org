...
The second is *sample size*: all the factors discussed above depend on dense and repeated sampling of the parameter space, which is by no means self-evident in actual animal experiments.
...
Curiously, whereas modeling approaches are generally about the quantification of effect size, /predictive/ modeling might link back to difference tests.
