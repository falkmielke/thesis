...
With the regression outcome, it is possible to convert any voxel gray value to densities, which applies for example to the femur.


** Issue 0: Mass Mismatch and Total Mass Normalization

To confirm the outcome of the regressions above, we compared the actual mass of the femur (which is \(5.271 g\)) to the outcome of the digital measurement.
...
With the linear regression reported above, the femur mass is calculated to weigh \(6.38 g\) (\(+21 \%\)).
The exponential regression yields a lower than actual mass (\(4.96 g\), \(-29 \%\)).
Though it appears that the correct regression would lie somewhere between the exponential and linear attempt, there is no data or theoretical justification supporting any intermediate hypothesis; there is no reason to think that all biological tissues generally fall in between the presented cases.

We conclude that, at least for a femur, the regression method is not accurate.


To exclude that problems arose from the composition of this isolated bone, we applied the regression method to more complex data.

In a second experiment, a piglet body was CT scanned, virtually segmented, and analogously dissected to see whether the total mass of the virtual segments matches the actual masses.
This scan was performed at a different facility and prior to the density calibration experiments, and thus the only available calibration points were air, water, and two density calibration phantoms.
Consequently, an exponential regression was used to convert gray values to density.
Although an exponential regression was used, this virtual density conversion leads to a considerable over-estimation of the mass of most of the segments, and the specimen total (as opposed to the under-estimation in case of the femur exponential regression).

This result once again highlights how erratic the digital density conversion is.



#+NAME: tab:pigletdissection
#+ATTR_LATEX: :environment tabular :align |l|c|c|c|
#+CAPTION: Segment mass verification on an actual piglet specimen. Note that the total mass at euthanasia was measured \(1.125 kg\), a slight loss is expected due to condensation and preservation.
#+begin_table
|-----------------+-----------------+------------+------------|
...
|------------------------+-------------+----------------+-----------+---------------------+---------------------+---------------------|
| *COM \(x\mid y \mid z\)* | \(+2.5 mm\) |           6.38 |        50 |                0.52 |                1.36 |                1.67 |
|------------------------+-------------+----------------+-----------+---------------------+---------------------+---------------------|
...
Shifting the COM alone does not affect inertial parameters, which is expected.
However, this does not incorporate errors in an inverse dynamics workflow, where a COM shift will translate to a joint wrench difference according to the Steiner law.
Adjusting the mass expectably has the same effect as a change in regression parameters, and a \(5\%\) mass increase causes a \(\Delta I\) of more than \(8\%\).
...
These simulations confirm that moment of inertia crucially depends on the mass that is assigned to each voxel in the CT data set.
...
(i) the polychromatic character (source spectrum) of the used radiation, (ii) chemical composition (absorption spectrum), and (iii) scan artifacts
