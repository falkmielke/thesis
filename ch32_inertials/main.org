...
X-ray sources are "targets", i.e. anodes made of certain metals (Tungsten, Molybdenum, ...), shot at with an electron beam which is evoked by applying voltage (e.g. \(60\ kV\)) to the warmed-up anode and kathode.
...
Nevertheless, comparing the calibration line and the DECT ratio results, the putative outliers do not stand out more than other regression elements (especially the PTFE material puts the original regression in doubt: it is perfectly fit by the calibration regression, is a modest outlier on the DECT ratio, despite a special chemical composition containing fluorine).

For a toy pig of unknown plastic material, citet:DuPlessis2013 retrieve higher than actuall mass estimates; they identify chlorine and calcium content as the cause.
...
It should be pointed out that preliminary (naïve) results of this study were first presented at the [[http://mielke-bio.info/falk/posts/26.seb2021][conference of the Society of Experimental Biology (SEB) in 2021]], one year prior to the citet:Durston2022 study.
...
More generally, limb segments which are treated as a unit have to be labeled in dedicated software (e.g. [[https://www.slicer.org][3D Slicer]]), typically with a kind of "color brush" or "magic wand" tool; thresholding of grey values, boolean operations, algebraic operations ("filling" etc.), and other tricks may simplify the segmentation procedure in certain situations.
...
Although an exponential regression was used (under-estimation in case of the femur), this virtual density conversion leads to a considerable over-estimation of the mass of most of the segments, and the specimen total (average \(+12 \%\)).
