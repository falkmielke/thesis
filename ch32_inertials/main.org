#+TITLE: The Inaccuracy of Approximating Inertial Properties of Biological Specimens from X-Ray Computed Tomography
#+AUTHOR: Falk Mielke
#+DATE: <2023-07-13 Thu>
# <2023-07-05 Thu>-<2023-07-13 Thu>
...
This desire even increased by the development of computed tomography @@latex:\citep[CT;][]{Beckmann2006,Hounsfield1973}@@, a set of techniques which enable the reconstruction and visualization of three-dimensional structural images.
...
Notwithstanding this list of known problems, people have repeatedly attempted to extract density and inertial properties from CT scans @@latex: \citep[][as well as the present study]{Phillips1997,DuPlessis2013,Durston2022}@@.
...
#+CAPTION: \textbf{Spectra.} The horizontal axis shows all wavelengths \(\lambda\) (\(nm\)) of relevance (in this case the range of visible light, for illustration). Related to wavelength and therefore viable axis label alternatives are frequency (units: \(Hz\)) and photon energy (units: \(eV\)). The vertical axis shows photon occurrence probability, or normalized intensity \(I\), or the intensity difference \(\Delta I\) for the absorption spectrum. Black curve: emission spectrum of a white LED \citep{Tanabe2005}. Green curve: the approximate absorption spectrum of chlorophyll A \citep{Zscheile1934}. The chlorophyll will hardly get excited by the LED, yet the plant will look bright and green.
#+LABEL: fig:spectrum
#+ATTR_LATEX: :placement [p] :width 13cm
[[./figures/spectrum.pdf]]


To characterize light which is emitted, reflected, or absorbed, one can plot a spectrum (Fig. \ref{fig:spectrum})@@latex: \footnote{\chng{Patient readers of this thesis will see a connection to Fourier methods (Ch. \ref{cpt:fourier_review}).}}@@.
Red LEDs are an alternative to red filters: they immediately produce a spectrum which is biased towards low frequencies.
One can assess that red LEDs have a different *source spectrum or emission spectrum* than white LEDs or "vintage" (tungsten) light bulbs.
...
@@latex:\end{change}@@
X-ray sources are "targets", i.e. anodes made of certain metals (Tungsten, Molybdenum, ...), shot at with an electron beam which is evoked by applying voltage (e.g. \(60\ kV\)) to the warmed-up anode and cathode.
...
Much like a monochrome digital camera photo cell measures light intensity of any wavelength within the visual range (restricted to /visual/ by another filter); in contrast, a truly monochromatic detector would detect only a specific wavelength.
...
*"Attenuation"* is the term to describe that not all photons reach the detector, i.e. the detected x-ray intensity is lower than the incident intensity.
...
The term "artifact" @@latex:\chng{linguistically implies\footnote{Artifact: ``A product of artificial character (as in a scientific test) due usually to extraneous (such as human) agency''; Merriam-Webster online dictionary, ; https://www.merriam-webster.com/dictionary/artifact. ``A false or inaccurate result or effect that is produced accidentally''; The Britannica Dictionary; https://www.britannica.com/dictionary/artifact. Accessed February 6th, 2024.}}@@ that there is an "error" or flaw in the measurement, yet instead it can be ascertained that correctly obtained x-ray images are highly accurate.
...
These algorithms are constantly refined and improved, and specific algorithm variants can already overcome specific scan limitations @@latex:\citep[e.g.][]{Six2019,Frenkel2022}@@.
In the future, the question of reconstruction artifacts will rather be one of algorithm choice.
Today, the most common reconstruction algorithm, used in a vast majority of the CT service facilities with cone-beam setups, is "filtered backprojection" @@latex:\citep[FBP, or Feldkamp/Davis/Kress = FDK algorithm;][]{Feldkamp1984}@@.
...
I selected them because they are related to my own work, and to point out a unifying feature and a reason for the highlighted flaws in theses and other studies: they are /output driven/, and fall short of discussing the mechanistics of CT imaging.
...
I also attempted to validate the method on dissected animal specimens @@latex:\citep[as][but less sophisticated]{Durston2022}@@.
...
+ How (much) exactly do known artifacts contribute to the measurement error?
+ Are the cheap plastic parts appropriate calibration options for organic tissue?


It should be pointed out that preliminary (naive) results of this study were first presented at the [[http://mielke-bio.info/falk/posts/26.seb2021][conference of the Society of Experimental Biology (SEB) in 2021]], one year prior to the citet:Durston2022 study.


#+CAPTION: *Experimental Setup.* A piglet femur was excised an marked with metal bead markers. Together with various calibration objects, the sample was fixed ("mounted") in a PET bottle to fit into the cylindrical scan volume of [[https://www.uantwerpen.be/en/research-groups/vision-lab/x-ray-imaging-resear/flexct][the FleXCT scanner of the University of Antwerp]]. The image on the left shows the sample mount, magnified on the inset. On the right is one CT projection of the sample, with the femur with markers clearly visible on top.
#+LABEL: fig:femur_scan
#+ATTR_LATEX: :placement [b!] :width 16cm
[[./figures/femur_scan.jpg]]
...
Procedures have been approved /a priori/ by the Ethical Committee of Animal Experimentation, University of Antwerp, Belgium (approval number 2017–25).
...
#+CAPTION: \textbf{The "flying femur" experiment:} an excised piglet femur used in XROMM experiments as an allegory of reductionism. Left: the femur oscillating on a pendulum. Right: installation with a motor capable of moving the femur in a realistic way.
...
Biplanar x-ray video recordings were performed on the [[https://www.uantwerpen.be/3d2ymox][University of Antwerp's 3D²YMOX system]] citep:Nguyen2021,Sanctorum2020, with two experimental settings.
...
They calibrate bone /mineral/ density\footnote{Bone Mineral Density (BMD) quantifies the amount of inorganic content within a volume of bone tissue. Though sharing similar units with physical density, BMD measures just a fraction of the physical bone density.} of \(0.25\ \frac{kg}{m^3}\) and \(0.75\ \frac{kg}{m^3}\), and they have a /physical/ density of \(1254\ \frac{kg}{m^3}\) and \(1485\ \frac{kg}{m^3}\).
...
#+CAPTION: \textbf{Virtual dissection (segmentation) of a piglet CT scan.} Example virtual slice.
...
#+ATTR_LATEX: :placement [p] :environment tabular :align |l|c|
#+CAPTION: \textbf{CT scan settings} for the calibrated femur scan.
|------------------+--------------------|
| voltage          | \(150\ kV       \) |
| power            | \(55\ W         \) |
...
All code used on this project, including Python implementations of the mathematical formulas below, can be [[https://git.sr.ht/~falk/flying_femur][found online]] (\nolinkurl{https://git.sr.ht/~falk/flying_femur} and \nolinkurl{http://mielke-bio.info/falk/posts/23.ct_density}).
...
More generally, limb segments which are treated as a unit have to be labeled in dedicated software (e.g. [[https://www.slicer.org][3D Slicer]]), typically with a kind of "color brush" or "magic wand" tool; thresholding of gray values, boolean operations, algebraic operations ("filling" etc.), and other tricks may simplify the segmentation procedure in certain situations.
...
... with the rigid body's volume \(V\) split up into voxels \(i\) which have mass \(m_i\) and position vector \(\vec{r}_{i}\); \(E_{3}\) is the \(3\times 3\) identity matrix and \(\otimes\) the outer product.
...
The reference point of \(I\) can be changed by the generalized parallel axis / Steiner's theorem @@latex:\citep[][p. 245]{Lynch2017}@@.
#+begin_src latex
\begin{equation}\label{eqn:steiner}
 I_{p} = I_{0} + m \cdot \left( \vec{s}^{T} \vec{s} E_{3} - \vec{s} \vec{s}^{T} \right)
\end{equation}
#+end_src
... with \(\vec{s}\) being the shift vector connecting the original and target points.
...
For symmetrical objects \chng{which rotate around the center of symmetry, \(I\)} is independent of the total mass.
# was wrong, see https://youtu.be/XPUuF_dECVI
Under controlled experimental conditions, a homogeneous aluminium cylinder will roll down a slope in exactly the same time as a homogeneous lead cylinder of identical form and size. # for different reasons: mass cancels due to the "pure roll" condition.
...
#+ATTR_LATEX: :placement [b] :environment tabular :align |l|l|l|
#+CAPTION: \textbf{Density Regression Functions} and regression start values.
|-------------+---------------+------------------------------|
...
This function is linear in \(I\), so error propagation is quite simple@@latex:\footnote{It should not be assumed that the error in measuring \(\frac{d^2\theta}{dt^2}\) is zero, but I will here focus only on the \(I\) component.}@@ citep:Normann2016:
...
This is a numerical *"sensitivity analysis"*.
...
To estimate the uncertainty in the density regression, I repeated the procedure in a probabilistic modeling framework @@latex:\citep[PyMC, Version 5.6,][]{Salvatier2016}@@.
...
+ confirm results of the conventional least squares regression
+ provide a measure for the intercept uncertainty (\(\varepsilon\))
+ identify potential slope uncertainties (\(\varepsilon_{j}\))
...
#+ATTR_LATEX: :placement [p] :environment tabular :align |l|c|c|c|
#+CAPTION: \textbf{Segment mass verification on an actual piglet specimen.} On the lighter segments, measurement accuracy and rounding are limiting. Note that the total mass at euthanasia was measured \(1.125\ kg\), a slight loss is expected due to condensation and preservation.
|----------------+-----------------+------------+------------|
...
If there are *any beam hardening or streak artifacts*, which is non-trivial to remove from FBP reconstructed scans, *the moment of inertia will be incorrect*.
...
#+CAPTION: \textbf{Metal bead artifacts,} marked with orange arrows. Despite the use of an aluminium filter to harden the x-ray beam, metal beads produce conspicuous, star-shaped streaks on the scan reconstructions. The reason that the center of the metal beads appears black is a clipping setting during image preprocessing (intentional, do ensure they do not contribute to the total mass).
...
#+ATTR_LATEX: :placement [p] :environment tabular :align |l|l|c|c|c|c|c|
#+CAPTION: \textbf{Sensitivity analysis of the mass moment of inertia calculations of the femur scan.} For reference, the actual femur weighed \(5.271\ g\), had a length of \(46\ mm\), volume of \(4.637\times 10^{-6}\ m^{3}\) and an average density of \(\approx 1140\ kg m^{-3}\). Regression \(a\) and \(b\) are fit parameters (see text).
|----------------------+--------------+----------------+-----------+---------------------+---------------------+---------------------|
| *parameter*            | *input*        | *\(\sum_{i}m_{i}\)* | *\(\Delta \vec{COM}\)* |          *\(I_{xx}\)* |          *\(I_{yy}\)* |          *\(I_{zz}\)* |
|                      | *change*       |        \([g]\) |  \([px]\) | \([\times 10^{-6} kg m^2]\) | \([\times 10^{-6} kg m^2]\) | \([\times 10^{-6} kg m^2]\) |
|----------------------+--------------+----------------+-----------+---------------------+---------------------+---------------------|
|----------------------+--------------+----------------+-----------+---------------------+---------------------+---------------------|
...
|----------------------+--------------+----------------+-----------+---------------------+---------------------+---------------------|
| *\(\vec{COM}\) \(x\)*    | \(+2.5\ mm\) |           6.38 |        50 |                0.52 |                1.40 |                1.71 |
| *\(\vec{COM}\) \(y\)*    | \(+2.5\ mm\) |           6.38 |        50 |                0.56 |                1.36 |                1.71 |
| *\(\vec{COM}\) \(z\)*    | \(+2.5\ mm\) |           6.38 |        50 |                0.56 |                1.40 |                1.67 |
|----------------------+--------------+----------------+-----------+---------------------+---------------------+---------------------|
...
The ideal object to calibrate an unknown femur would thus be an identical femur of known mass distribution @@latex:\citep[which is actually attempted in clinical tomography, cf.][]{Lennie2021}@@.
...
I also evaluated that scaling voxel-wise density, so that the mass matches the known total mass, is not an option: this modification can amplify issues from beam hardening (Fig. \ref{fig:cupping_results}) or from marker streaks and thus make the outcome more unpredictable.
...
For grounded limbs, the inertial properties of the individual limb segments themselves are negligible (order of magnitude of limb segment mass /versus/ total mass), although, depending on the behavior under study, accelerations might be high and caution might be appropriate.
...
Segments /in vivo/ are not rigid.
...
# This is not an issue due to the common practice of splitting limbs into anatomical segments at the major joints (Ch. \ref{cpt:dynamics}).

Another factor is the preservation of the specimen (frozen in this case; formol, alcohol, staining, ...) which has an influence on the chemical composition and inertial properties (not assessed herein).
...
The main challenges of this approach are the workload of segmentation, and again transferability to /in vivo/ measurements


Finally, an alternatives to CT scans for the present purpose might be /better/ CT scans.
...
https://doi.org/10.1242/jeb.245132 review!
