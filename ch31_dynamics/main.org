...
#+CAPTION: A hypothetical, scientific experiment to illustrate dynamic modeling (see text). Upper left: experimenter setting a door in motion. Lower right: subject running towards the experimental setup. \(\vec{P}\): hinge point/axis of the door, \(\vec{v}\): velocity of the subject, \(\dot\theta\): angular velocity of the door, \(\vec{F}\): force applied to the door by the experimentor.
#+LABEL: fig:slamming
...
All these possible outcomes are manifestation of the *conservation of energy and momentum*, which I will explore \chng{on the metaphor of a} hypothetical door slamming experiment.
...
Until this point, this thesis focused on methodological development in terms of measuring, modeling and predicting /kinematics/.
...
This opens up a rabbit hole of elementary physics @@latex:\citep["Erhaltungssätze", \textit{sensu}][]{Noether1918}@@:
#+begin_quote
...
# TODO: Peter: "the change of momentum is dynamics" but momentum of the universe does not change; conservation of momentum holds on open systems (most definitions are wrong, NASA is right); impulse-momentum-theorem not a theorem: impulse is just defined as the change of linear momentum over time (temporal integral, [[https://en.wikipedia.org/wiki/Impulse_(physics)][https://en.wikipedia.org/wiki/Impulse_(physics)]] ).
...
In a nutshell:
+ Wrenches and quaternions are useful computational tools for calculating rigid body dynamic models.
+ Balance Equations are the mathematical manifestation of conservation of momentum.
+ Reference frame considerations can simplify calculations; however, static frame calculations are sufficient and result in correct joint moments.
...
+ Computed Tomography might be a shortcut to approximate structural properties which are crucial for the calculation of dynamics (next chapter).
...
** Forces
Forces are the product of mass and an acceleration, \(\vec{F}=m\ddot \vec{x}\) (or, more familiar to most, \(\vec{F}=m\vec{a}\)), which is one of Newton's laws of motion citep:Newton1687,Tipler2007.
...
Linear momentum is defined as follows: \(\vec{p}=m\dot \vec{x}\), with \(m\) the mass and \(\dot \vec{x}\) the linear velocity (change of position over time).

Combining these equations and applying the total derivative:
\begin{change}
\[\frac{d\vec{p}}{dt} = m \cdot \frac{d\dot \vec{x}}{d t} + \frac{d m}{d t} \cdot \dot \vec{x}\]
\end{change}
...
\vec{F} = m \cdot \ddot \vec{x} + \frac{d m}{d \vec{x}} \cdot \dot \vec{x} \dot \vec{x}
\end{equation}
#+end_src



Because \(\vec{a}=\ddot \vec{x}\), the first part of the right handside expression in equation @@latex:\eqref{eqn:force}@@ is the familiar \(\vec{F}=m\vec{a}\).
The second part is more enigmatic: those are "velocity product terms", which involve partial derivatives of the mass matrix@@latex:\footnote{Mass is often taught as a scalar quantity, yet in more complex problems of composite systems, which have a mass distribution, it can be more useful to represent mass by a matrix/tensor. This is analogous to the mass moment of inertia.}@@, thereby accounting e.g. for configuration changes of a chain of segments, deformations or mass shifts (e.g. muscles, blood flow, ...).
...
Hence, we get a more or less familiar equation for the Force \(\vec{F}\), which tells us that the acceleration of a mass (\(m\ddot \vec{x}\)), i.e. a change in linear momentum, must be equal to an external force.
\(\vec{F}\) is a three element vector.


** Moments

#+CAPTION: Force and Moment. A force applied further away from the hinge \(\vec{P}\) (situation A) will generate a larger \chng{moment} than a force similar in magnitude, but applied more proximal to the hinge (situation B). The reason is that the moment is defined as the cross product of the position vector of the force application point (in a hinge-centered reference frame) and the force vector. \chng{Furthermore, because the change in angular momentum is related to that moment, the door will gain a higher momentum (and will therefore "hit harder").}
...
Analogously, Mass Moment of Inertia@@latex:\footnote{Note that \(I\) is a matrix, usually 3-by-3; this is analogous to mass, which in its general form for complex objects is also a matrix \citep{Lynch2017}.}@@ (\(I\)) is the resistance of an object to rotational acceleration, which induces a change in angular momentum \(\vec{L}\).
...
And so, the series of equations above can be applied analogously to angular momentum \(\vec{L}\) (with angular velocity \(\dot \theta\)).
\[\vec{L} = I \dot \theta\]

#+CAPTION: Derivation of Newton-Euler equation of rotational motion.
#+LABEL: fig:miracle
#+ATTR_LATEX: :placement [p] :width 8cm
[[./figures/miracle_occurs.jpg]]



The time derivative is as follows.
\begin{change}
\[\frac{d\vec{L}}{dt} = I \cdot \frac{d \dot \theta}{d t} + \frac{d I}{d t} \cdot \dot \theta\]
\end{change}

The time derivative of angular momentum is called moment (\(\vec{M}\); see also: citet:WikipediaNewtonEuler,WikipediaTorque), and with further manipulation @@latex:\citep[details in][p. 242, eqn. 8.23; otherwise see Fig. \ref{fig:miracle}]{Lynch2017}@@ yields:
#+begin_src latex
\begin{equation}\label{eqn:moment}
\vec{M} = I \ddot\theta + \dot\theta \times I\dot\theta
\end{equation}
#+end_src



#+CAPTION: The slamming door, isolated sketch.
...
There is another extra step compared to the linear momentum: calculation of the *angular momentum requires a reference point* (usually the hinge, \(\vec{P}\), as above, but any point could be chosen).
...
The trick is to concatenate the vectors \(\vec{F}\) and \(\vec{M}\) to form a six element vector \(\vec{W}\), *the Wrench* (Fig. \ref{fig:wrench}, cf. https://en.wikipedia.org/wiki/Screw_theory).
...
#+CAPTION: The door slamming experiment in wrench notation (see text). Point \(\vec{P}\) is the origin of the reference frame, \(\vec{r}_{\vec{PD}}\) is the position vector to the attack point \(\vec{D}\) of the wrench \(\vec{W}\).
...
Plugging in the non-zero part of equations \eqref{eqn:force} and \eqref{eqn:moment} from above, we get an expression for what can be referred to as the *dynamic wrench* (defined at the \vec{COM}):
#+begin_src latex
\begin{equation}\label{eqn:dynamic_wrench}
\vec{W}_{dyn} = \left[ m\ddot \vec{x}; I\ddot \theta \right]
\end{equation}
#+end_src
...
\bar \vec{W}_{dyn} = \vec{W}_{dyn} + [ \vec{0}; \vec{s} \times \underbrace{(m\ddot \vec{x})}_{\vec{F}} ]
\end{equation}
\end{change}
#+end_src
...
On the other handside of the equation, we have any *external wrenches* acting on the object.
...
** Quaternions
\begin{change}
#+CAPTION: Spatial rotation with *Quaternions*. A point \(\vec{D}\) (blue) can be rotated by an angle \(\theta\) around an axis \((x; y; z)\). The higher the rotation angle value, the further the rotation. A quaternion (large black arrow) can encode both the axis and the angle in a 4D vector, in a manner which conveniently enables the execution of the rotation by simple vector multiplication.
#+LABEL: fig:quaternion
#+ATTR_LATEX: :placement [p] :width 17cm
[[./figures/quaternions.pdf]]
\end{change}
...
+ The Balance of Wrenches, equation \eqref{eqn:balance_of_wrenches}, can be calculated entirely in the inertial reference frame, without repeated switch to joint coordinate systems (and return).
+ Wrenches can easily be moved to other reference points via equation \eqref{eqn:shift_wrench}.
+ A convenient point to calculate the balance of wrenches is the center of mass (\(\vec{COM}\)); rotation about that point is an expression of spin angular momentum (an intrinsic property) and most components simplify (e.g. gravitational wrench \(\vec{W}_{\vec{g}}\), dynamic wrench \(\vec{W}_{dyn}\)).
...
Dumas /et al./ have provided and since then improved an implementation of their procedure in Matlab citep:DumasMatlab.
...
The inertial wrenches (gravitational wrench, friction, ...) are known; the dynamic wrench is measured from kinematics and inertials; yet connecting wrenches are usually unknown: each segment (e.g. the thigh) receives an unknown wrench input from all adjacent segments (e.g. the pelvis and the lower leg).
...
The proximal joint wrench is shifted to the proximal joint position, its natural "point of attack"@@latex:\footnote{This series of shifts should have illustrated why the wrench is a useful construct: computational rigor can make sure that the reference points are respected.}@@.
...
Gravity pulls soft tissue into a direction citep:Hansraj2022 which might be different /in vivo/ and on the preserved configuration of the limb.
...
All this reduces congruence of the preserved, measured body part and the /in vivo/ situation: mass is constant, yet the \(\vec{COM}\) of a segment is not stable during locomotion, and the mass moment of inertia tensor will most certainly vary during the process.
...
By /post mortem/ fixation and dissection, or by \chng{digital volumetric segmentation}, segments can be separated and their inertial properties measured.
