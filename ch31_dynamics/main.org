...
** Balance of Wrenches
Plugging in the non-zero part of equations \eqref{eqn:force} and \eqref{eqn:moment} from above, we get an expression for what can be referred to as the *dynamic wrench*:
#+begin_src latex
...
+ The Balance of Wrenches, equation \eqref{eqn:balance_of_wrenches}, can be calculated entirely in the inertial reference frame, without repeated switch to joint coordinate systems (and return to ICS).
...
The inertial wrenches (gravitational wrench, friction, ...) are known.
