...
Because \(a=\ddot x\), the first part of the right handside expression in equation @@latex:\eqref{eqn:force}@@ is the familiar \(F=ma\).
...
Analogously, Mass Moment of Inertia@@latex:\footnote{Note that \(I\) is a matrix, usually 3-by-3; this is analogous to mass, which in its general form for complex objects is also a matrix \citep{Lynch2017}.}@@ (\(I\)) is the resistance of an object to rotational acceleration, which induces a change in angular momentum \(L\).
...
The time derivative of angular momentum is called moment (\(M\); see also: citet:WikipediaNewtonEuler,WikipediaTorque), and with further manipulation @@latex:\citep[details in][p. 242, eqn. 8.23; otherwise see Fig. \ref{fig:miracle}]{Lynch2017}@@ yields:
#+begin_src latex
...
There is another extra step compared to the linear momentum: calculation of the *angular momentum requires a reference point* (usually the hinge, \(P\), as above, but any point could be chosen).
...
Plugging in the non-zero part of equations \eqref{eqn:force} and \eqref{eqn:moment} from above, we get an expression for what can be referred to as the *dynamic wrench* (defined at the COM):
#+begin_src latex
...
+ A convenient point to calculate the balance of wrenches is the center of mass (\(COM\)); rotation about that point is an expression of spin angular momentum (an intrinsic property) and most components simplify (e.g. gravitational wrench \(W_{g}\), dynamic wrench \(W_{dyn}\)).
