...
If a person in question is running at constant velocity (which we could quantify using Fourier methods and probabilistic modeling, see before), the unexpected appearance of an obstacle will certainly change the subject's trajectory, and in unfortunate, but most probable cases also *change the subject's momentum*.
The non-sliding door itself, set up on a hinge (point \(P\) in Fig. \ref{fig:slamming}), will gain rotational momentum through the act of slamming.
It will subsequently transfer inertia to the subject in what physicists call a collision, thereby ultimately canceling out the inertia of the person if the impact is in opposite direction and approximately equal magnitude to the running.
More generally, the subject might (i) decelerate but proceed, (ii) come to a hold, or (iii) ungently bounce backwards from the elastic collision.
...
All these possible phenomena are due to the conservation of energy and momentum, which we will explore below.
...
Experienced biomechanicists are welcome to skip to the next chapter.
...
These things seem quite intuitive: the slamming door will impact the subject upon impact.
...
These conversions and transfers are precisely the reason /why segments or subjects change their movement/, and we seek to quantify and model (i.e. predict) them.
...
Therefore, this chapter will summarize the theory behind Inverse Dynamics, and point at sources and conventions that turned out to be useful for the purpose of this thesis.


* Dynamics: Theory
** Forward and Inverse Problems
Since the "slamming door" experiment could be unpleasant for the faces of living subjects, one might be tempted to measure on a robot.
...
In fact, this simple physical observation explains why handles are where they are.
...
Other points require more complex calculations, except in the special case where rotation happens about the center of mass (\(COM\)).
...
A mathematical shorthand for writing wrenches are square brackets and a semicolon indicating that the components are concatenated:
#+begin_center
\(W = \left[ F; M \right]\)
#+end_center
...
The \(x\) and \(theta\) in the dynamic wrench refer to the movement of the rigid body itself.
...
Any other external forces and moments should be added in wrench form.
...
Transformation from one to another reference frame is possible by matrix multiplitation with a transformation matrix.
...
Angular momentum depends on angular acceleration, which is a temporal derivative of "angle".
...
There is one exception: the ground contact.
...
The contact point, in calculating the segment which is in ground contact, is also called the tip wrench; trivially, it is zero if the distal segment is lifted off the ground.
One can calculate the wrench on the proximal joint by subtracting from the dynamic wrench of the segment (measured linear and angular movement) the known external wrenches (ground contact wrench, gravitational wrench, ...; all shifted to the \(COM\)).
...
** Inertial Properties
The procedure is quite trivial to implement, however the measurement uncertainty is not in homogeneous.
...
The classical method requires a dissection of the segment; this can be a precise anatomical disarticulation or slicing of a frozen specimen.
The segment mass is then measured on a simple scale.
Center of mass could be derived from a classical pendulum experiment: hang an object from a thread, measure oscillation period, apply \(T=2\pi\sqrt{\frac{l}{g}}\) solved for effective length \(l\), repeat the procedure with the object hung from a different axis.
A common simplification is to give the position of the \(COM\) as a fraction of the segment length, inaccurately assuming that it lies on the segment long axis.
...
The pendulum measurements above can yield most accurate results.
However, the dissected limb is only a limited representation of the segment /in vivo/.
...
Formol, ethanol, or temperature are typical agents used as a fixative; any fixative has its effect on soft tissue citep:Buytaert2014.
This issue can putatively change all inertials properties of the sample.

Fourth, 3D volumetric imaging (e.g. CT scans) could be an option to replace the measurements above, yet they come with their own set of problems and limitations.
...
* Summary
This chapter serves as a brief overview of the method of inverse dynamic calculations.
It outlines the overall procedure, and documents the specific procedure that the author applied for inverse dynamic measurement of piglet locomotion.
