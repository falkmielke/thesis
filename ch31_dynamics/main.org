...
# Experienced biomechanists who are also familiar with "screw theory" are welcome to skip to the next chapter.
...
Because \(\vec{a}=\frac{d^2\vec{x}}{dt^2}\), the first part of the right hand side expression in equation @@latex:\eqref{eqn:force}@@ is the familiar \(\vec{F}=m\vec{a}\).
...
#+CAPTION: \textbf{Derivation of Newton-Euler equation of rotational motion.}
#+LABEL: fig:miracle
...
#+CAPTION: \textbf{The slamming door, isolated sketch.}
#+LABEL: fig:slamming_momentum
#+ATTR_LATEX: :placement [b!] :width 12cm
...
#+CAPTION: \textbf{The door slamming experiment in wrench notation (see text).} Point \(\vec{P}\) is the origin of the reference frame, \(\vec{r}_{\vec{PD}}\) is the position vector to the attack point \(\vec{D}\) of the wrench \(\vec{W}\).
...
On the other hand side of the equation, we have any *external wrenches* acting on the object.
...
** Quaternions
#+CAPTION: @@latex:\chng{\textbf{Spatial rotation with quaternions}. A point \(\vec{D}\) (blue, with gray dashed position vector) can be rotated by an angle \(\theta\) around an axis \((x; y; z)\) to position \(\vec{D}_{rot}\). The higher the rotation angle value (indicated by the orange area), the further the rotation. A quaternion (large black arrow) can encode both the axis and the angle in a 4D vector, in a manner which conveniently enables the execution of the rotation by simple vector multiplication.}@@
#+LABEL: fig:quaternion
#+ATTR_LATEX: :placement [b] :width 17cm
...
When we think about rotation in space, we might think about an axis and an angle @@latex:\chng{(Fig. \ref{fig:quaternion})}@@.
...
#+CAPTION: \textbf{Measuring inertial properties of the slamming door:} the center of mass.
